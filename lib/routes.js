...
const whitelistBackToPathReturnUrl = [
  ...
  `post_logout_redirect_uri=${process.env.APPLICATION_DOMAIN_URL}/business/email-address-updated?email=`
]
    ...
    handler: async (request, h) => {
      try {
        ...
        if (
          ...
          !whitelistBackToPathReturnUrl.some(entry => redirectPath.includes(entry))
        ) {
