  ...
  server.route({
    ...
    handler: async (request, h) => {
      try {
        ...
        let url;
        if(redirPath.indexOf('http') > 0) {
          url = new URL(redirPath);
        } else {
          const currentHost = request.connection.info.protocol + '://' + request.connection.info.host;
          url = new URL(redirPath, currentHost);
        }

        const sanitised = redirPath.replace(url.protocol + '//', '').replace(url.host, '');

        return h.redirect(sanitised || '/')
      } catch (e) {
        debug(e)
